...
def _try_import_pymem():
    try:
        ...
        raise MemoryReaderError(
            "pymem fehlt. Installiere mit: pip install pymem"
        ) from exc
...
class StatMemoryReader:
    ...
    def attach(self) -> bool:
        with self._lock:
            ...
            raise MemoryReaderError(
                "Monster Prom läuft nicht oder keine Rechte (als Admin?). "
                f"Fehler: {last_error}"
            )
    ...
    def read_stats_auto(
        ...
        try:
            ...
            if fallback and any(int(fallback.get(k, 0)) for k in STAT_KEYS):
                ...
                return (
                    None,
                    "Stats nicht im RAM gefunden — Werte exakt aus dem "
                    "Spiel eintragen (Spiel muss laufen).",
                )
            ...
            return None, f"Memory-Fehler: {exc}"
