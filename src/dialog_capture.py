    ...
    *,
    ...
    if not hwnd:
        return {
            ...
            "option1": "",
            "option2": "",
            "error": "Monster-Prom-Fenster nicht gefunden — Spiel sichtbar, nicht minimiert?",
        }
    ...
    if full_window:
        ...
        if img:
            attempts.append(("fenster", img))
    for label, capturer in (
        ("antworten", capture_options_image),
        ("dialog", capture_dialog_image),
        ("fenster", capture_window_image),
    ):
    ...
    if not attempts:
        return {
            ...
            "option1": "",
            "option2": "",
            "error": "Screenshot fehlgeschlagen (Fenster zu klein?)",
        }
    ...
    if not best or not best.get("ok"):
        err = "; ".join(errors[:2]) if errors else "Kein Text — beide Antworten voll sichtbar?"
        return {
            "ok": False,
            "text": "",
            "lines": [],
