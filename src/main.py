...
def load_config(project_root: Path) -> dict:
    ...
    if example_path.exists():
        ...
        print(f"config.json erstellt aus Vorlage: {config_path}")
        return data
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Monster Prom Helper — trackt Events & Secret Endings im Hintergrund."
    )
    parser.add_argument(
        "--game",
        default=None,
        help="Spiel (Monster Prom, Monster Camp, Monster Prom 2)",
    )
    parser.add_argument(
        "--scan-mplogs",
        action="store_true",
        help="MPLogs einmalig scannen (nach Spielende)",
    )
    parser.add_argument(
        "--overlay",
        action="store_true",
        help="Dialog-Overlay-Fenster starten (Stats + Optionen)",
    )
    ...
    if args.scan_mplogs:
        print(f"Scanne MPLogs: {mplogs_path}")
