...
class EventDatabase:
    ...
    def match_screenshot(
        ...
        if combined:
            ...
            if best:
                return best, hits, "ok", "Volltext-Suche"
            if hits:
                return None, hits, "mehrere_moeglich", "Mehrere Treffer — Liste prüfen"
            hits = self.search_dialog(combined, limit=10)
            if len(hits) == 1:
                return hits[0], hits, "ok", "Stichwort-Suche"
            if hits:
                return None, hits, "mehrere_moeglich", "Stichwort — Liste prüfen"

        return None, [], "kein_treffer", "OCR-Text passt zu keinem Event in der DB"
    ...
    def match_by_two_options(
        ...
        *,
        ...
        if not hits:
            msg = "Kein Event zu den erkannten Antworten"
            if has_lines:
                msg += f" — gelesen: „{a[:40]}“ / „{b[:40]}“"
            return None, [], msg
        ...
        return None, hits, "mehrere_moeglich"
