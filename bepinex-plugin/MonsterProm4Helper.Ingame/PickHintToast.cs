...
public static class PickHintToast
{
    ...
    private static void RebuildHint(LiveGameState live, EventDb db)
    {
        ...
        try
        {
            ...
            {
                var a = live.ExchangeOptions[0];
                var b = live.ExchangeOptions[1];
                _visible = true;
                _title = "Option 1: +" + a.GainStat + " / -" + a.LoseStat;
                _body = "Option 2: +" + b.GainStat + " / -" + b.LoseStat;
                if (live.EnginePick > 0)
                    _sub = "Spiel bevorzugt Option " + live.EnginePick;
                return;
            }
            ...
            {
                _visible = true;
                _title = "EMPFOHLEN: Option " + vm.RecommendedOption;
            ...
            {
                _visible = true;
                _title = "Spiel waehlt: Option " + live.EnginePick;
            ...
            {
                _visible = true;
                _title = "Tendenz: Option " + vm.RecommendedOption;
                _body = vm.RecommendedStat + " " + vm.RecommendedValue;
                _sub = "Kein klarer Vorteil";
            }
    ...
    public static void SyncUi()
    {
        if (!_visible)
        {
            NguiOverlayView.HidePickHint();
            return;
        }

        NguiOverlayView.ShowPickHint(_title, _body, _sub);
    }
