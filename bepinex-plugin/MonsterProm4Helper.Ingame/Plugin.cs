...
public class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        try
        {
            ...
            _toggleKeyAlt = Config.Bind("Overlay", "ToggleKeyAlt", KeyCode.F9, "Overlay ein/aus (Alternativ)");
            _secretAlerts = Config.Bind("Alerts", "SecretEndingToast", true, "Secret-Ending-Toast");
            _secretAlertsFirstOnly = Config.Bind("Alerts", "SecretEndingFirstOnly", true, "Secret nur einmal pro Route");
            _pickHint = Config.Bind("Alerts", "PickHint", true, "Pick-Hint ohne F8");
            ...
            Log.LogInfo("Helper v" + PluginInfo.PLUGIN_VERSION + " — NGUI-Overlay (F8/F9), Tick-Hook aktiv");
        }
        catch (Exception ex)
        {
    ...
    private void ApplyToggle(KeyCode viaKey, string source)
    {
        ...
        Log.LogInfo(OverlayOpen ? "Overlay AN (" + viaKey + ", " + source + ")" : "Overlay AUS (" + viaKey + ")");
    ...
    internal void NudgeStat(string key, int delta)
    {
        ...
        {
            Log.LogInfo("Stats +/- nur in der Con-Runde (MainGame, Spieler aktiv).");
            return;
        }
    ...
    private void RefreshOverlayData()
    {
        ...
        if (!_bridge.TryRead(out _live, out err))
            ...
            _status = "In der Con — warte auf Dialog";
        else if (_live.InPregame)
            _status = "Prolog — Tropes / Likes / Setup";
        else
