...
public class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        try
        {
            ...
            _pickHint = Config.Bind("Alerts", "PickHint", true, "Pick-Hint bei Events (ohne F8)");
            _drinkInfo = Config.Bind("Alerts", "DrinkInfo", true, "Ctrl+Linksklick auf Drink = Effekt anzeigen");

            if (_toggleKey.Value == _toggleKeyAlt.Value)
                _toggleKeyAlt.Value = KeyCode.F9;

            _status = "F8 = Overlay | Ctrl+Klick = Drink-Info";
    ...
    private void ApplyToggle(KeyCode viaKey)
    {
        ...
        try
        {
            ...
            Log.LogInfo("Overlay AN — Szene=" + _view.SceneName + ", DB=" + _db.EventCount + " Events");
        }
        catch (Exception ex)
        {
    ...
    private void RefreshOverlayData()
    {
        ...
        if (!_bridge.TryRead(out _live, out var err))
            ...
            _status = "Camp-Runde — warte auf Dialog";
        else
