...
public static class DrinkInfoHelper
{
    ...
    private static void ShowLookup(DrinkDb db, string drinkName)
    {
        ...
        if (rec == null)
        {
            _visible = true;
            _title = drinkName;
            _body = "Kein Eintrag in drinks_db.json";
            _sub = "Ctrl+Klick";
        ...
        _visible = true;
        _title = rec.Name;
        _body = MonoUtil.HasText(rec.Effect) ? rec.Effect : "Kein Effekt dokumentiert";
        _sub = MonoUtil.HasText(rec.Misc) ? rec.Misc : "Ctrl+Klick = Drink-Info";
...
internal static class DrinkHoverPatch
{
    ...
    public static void TryApply()
    {
        ...
        try
        {
            ...
            {
                Plugin.Log.LogWarning("Drink-UI-Typen nicht gefunden.");
                return;
            }
            ...
            if (count == 0)
            {
                Plugin.Log.LogWarning("Drink-Hooks nicht gefunden.");
                return;
            }

            _patched = true;
            Plugin.Log.LogInfo("Drink-Hooks aktiv (" + count + ").");
        }
