...
from .paths import game_data_dir, resolve_paths
...
class OverlayApp:
    def __init__(self, project_root: Path) -> None:
        ...
        data_dir = game_data_dir(self.config.get("game", "Monster Prom"))
        self.db = EventDatabase(data_dir / "events_db.json")
        self.predictor = EventPredictor(self.db)
        self.memory = StatMemoryReader(project_root / "memory_cache.json")

        self.root = tk.Tk()
        self.root.title("Monster Prom — Dialog helper")
        ...
        self._status = tk.StringVar(value="Start Monster Prom…")
    ...
    def _build_ui(self) -> None:
        ...
        stats_frame = ttk.LabelFrame(self.root, text="Stats (for recommendation)")
        stats_frame.pack(fill=tk.X, **pad)
        mem_row = ttk.Frame(stats_frame)
        mem_row.pack(fill=tk.X, padx=6, pady=2)
        ttk.Checkbutton(
            mem_row,
            text="Live stats (experimental)",
            variable=self._use_memory,
            command=self._on_memory_toggle,
        ).pack(side=tk.LEFT)
        ttk.Button(mem_row, text="Calibrate", command=self._request_calibrate).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(mem_row, text="Clear cache", command=self._clear_memory_cache).pack(
            side=tk.LEFT, padx=2
        )
        ...
        ttk.Label(stats_frame, text="Color:").pack(anchor=tk.W, padx=6)
        ...
        search_frame = ttk.LabelFrame(self.root, text="Find dialog")
        ...
        self._screenshot_btn = ttk.Button(
            scan_row,
            text="Screenshot → event (both choices)",
        ...
        ttk.Label(manual, text="Search:").pack(side=tk.LEFT)
        ...
        ttk.Button(manual, text="Search", command=self._apply_search).pack(side=tk.LEFT, padx=2)
        ttk.Button(manual, text="Clear", command=self._clear_live_dialog).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(
            manual,
        ...
        ttk.Label(pick_row, text="Hits:").pack(side=tk.LEFT)
        ...
        self._options_frame = ttk.LabelFrame(self.root, text="Options")
        ...
        ocr_note = (
            "Button: screenshot when both choices are visible."
            if self._ocr_ready
        ...
        ttk.Label(
            self.root,
            text=ocr_note + auto_note + " Optional: manual search.",
            wraplength=390,
            foreground="#555",
    ...
    def _on_memory_toggle(self) -> None:
        if not self._use_memory.get():
            self._memory_trusted = False
            self._mem_status.set("Live memory off — stats stay manual")
    ...
    def _auto_calibrate(self) -> None:
        ...
        if self._memory_calibrated or not self._use_memory.get():
            return
        self._mem_status.set("Calibrating…")
    ...
    def _clear_ram_blocklist(self) -> None:
        ...
        if finished_n:
            parts.append(f"{finished_n} finished (log)")
        summary = ", ".join(parts) if parts else "was empty"
        self._mem_status.set(f"RAM blocklist cleared ({summary})")
        self._status.set("Blocklist empty — try Refresh RAM or Screenshot")
    ...
    def _block_current_ram_event(self) -> None:
        ...
        if name:
            ...
            self._status.set(f"RAM ignored: {name} — type another event")
            self._mem_status.set(f"'{name}' blocked")
        ...
        if "event" in result:
            ...
            if found:
                ...
                self._mem_status.set(
                    result.get("mem_status") or "RAM refresh: no event found"
                )
    ...
    def _clear_memory_cache(self) -> None:
        ...
        self._mem_status.set("Cache empty — enter stats, then Calibrate")

    def _request_calibrate(self) -> None:
        snap = self._snapshot_ui()
        if not any(snap["stats"].values()):
            self._mem_status.set("Enter stats from the game")
            return
        self._mem_status.set("Searching memory…")
    ...
    def _request_dialog_scan(self, *, force: bool = False) -> None:
        ...
        if not self.memory.stats_address:
            if force:
                self._mem_status.set("Calibrate stats first (Calibrate button)")
            return
    ...
    def _on_ram_refresh(self) -> None:
        if not self.memory.stats_address:
            self._status.set("RAM refresh: stats address missing")
            self._mem_status.set("Calibrate first — is the game running? Overlay as admin?")
            return
        self._status.set("Reading event from RAM…")
        self._mem_status.set("RAM refresh (priority)…")
        try:
            self.memory.attach()
        ...
        try:
            if kind == "calibrate":
                ...
                if addr and self.memory.verify_stable_reads(
                    ...
                    result["mem_status"] = "Memory verified (stable)"
                    result["stats"] = snap["stats"]
                else:
                    self.memory.clear_cache()
                    result["mem_status"] = (
                        "Stats not in RAM — enter values exactly as shown in-game "
                        "(game running? admin?). Options: pick the event manually."
                    )
                return result
                ...
                if found:
                    ...
                    if force_refresh:
                        if usable:
                            ...
                            result["mem_status"] = (
                                f"RAM: {len(usable)} candidates — pick a hit: "
                                f"{preview}{extra}"
                            )
                        else:
                            blocked = ", ".join(sorted(self._ram_blocked)[:4])
                            hint = (
                                f" Only blocked names in RAM: {blocked}"
                                if blocked
                                else " No event name visible in RAM"
                            )
                            result["mem_status"] = (
                                "RAM refresh: no hit." + hint
                                + " → clear the blocklist or use screenshot"
                            )
                return result
    ...
    def _start_memory_worker(self) -> None:
        ...
        if self._dialog_ocr and self._ocr_ready:
            ...
            self._mem_status.set(
                "OCR missing: pip install mss Pillow winocr — then restart the overlay"
            )
    ...
    def _apply_ocr_match(self, name: str, snippet: str, hits: list[dict]) -> None:
        ...
        self._mem_status.set(f"Read: {snippet}")
        self._refresh_options()

    def _ocr_ambiguous(self, snippet: str, names: list[str]) -> None:
        if self._manual_event_lock:
            return
        self._mem_status.set(f"OCR uncertain — check hits: {', '.join(names[:3])}")
    ...
    def _on_screenshot_scan(self) -> None:
        ...
        self._status.set("Reading both choices from the game window…")
        ...
        def work() -> dict:
            ...
            if not detail.get("ok"):
                return {"error": detail.get("error") or "OCR failed"}
    ...
    def _apply_screenshot_result(self, result: dict) -> None:
        ...
        if best and status == "ok":
            ...
            self._status.set(f"★ Event from choices: {name}{extra}")
            return
        ...
        if hits:
            ...
            self._status.set(
                f"Screenshot: {len(hits)} hits — check the list"
            )
            self._refresh_options()
            return

        self._status.set("Screenshot: no DB hit")
        if match_hint:
    ...
    def _apply_search_pick(self) -> None:
        ...
        if name:
            self._set_active_event(name, "Pick")
    ...
    def _apply_search(self) -> None:
        ...
        self._search_hits = hits
        if not hits:
            self._status.set("No hit — try other words from the dialog")
            return
        ...
        if len(hits) == 1:
            self._set_active_event(names[0], "Search")
        else:
            self._set_active_event(names[0], f"Search (1/{len(hits)} — check hits)")
    ...
    def _apply_bootstrap(self, state: dict) -> None:
        ...
        if self._in_round:
            if self._dialog_ocr and self._ocr_ready:
                self._status.set("Dialog open → OCR reads text from the game automatically")
            else:
                self._status.set("During dialog: type words from the text (see Search)")
                self._focus_event_entry()
            if self.memory.stats_address:
                self.root.after(200, self._snapshot_ram_presence)
                self.root.after(400, self._request_dialog_scan)
        else:
            self._status.set("No active round in the log — play a week or enter an event manually")
        self._refresh_options()
    ...
    def _refresh_options(self) -> None:
        ...
        if not self._in_round and not ev:
            ...
            self._options_text.insert(
                tk.END,
                "No round detected in the log.\n"
                "Start a week — or enter an event manually.\n"
                "(The overlay also reads existing log lines on startup.)",
            )
        ...
        if not ev:
            ...
            self._options_text.insert(
                tk.END,
                "Dialog open?\n\n"
                "1) Type words from the dialog above\n"
                "   (choice 1/2 text, or “Damien”, “party”, …)\n"
                "2) Multiple hits? → pick the right event from the list\n"
                "3) Or wait a moment — auto-detect (RAM)\n\n"
                "You cannot see the internal event name in-game.",
            )
            self._options_text.config(state=tk.DISABLED)
            return
        ...
        if pick and pick.get("verdict") == "success":
            ...
            self._ctx_pick.set(
                f"Option {pick['option']} ({pick['stat']}) — close, value {pick['value']}"
            )
        else:
            self._ctx_pick.set("")
        ...
        if pick:
            lines.append(f">>> PICK OPTION {pick['option']} <<<")
            lines.append(f"    {pick['stat']} — value {pick.get('value', '?')}")
            if pick.get("hint"):
    ...
    def _on_log_line(self, line: str) -> None:
        ...
        def update() -> None:
            if parsed.kind == "scene_load" and parsed.scene:
                ...
                if in_school:
                    self._status.set("During dialog: type words from the game text into Find dialog")
                    self._focus_event_entry()
                    if self.memory.stats_address:
                    ...
                    self._clear_live_dialog()
                    self._status.set("Menu / pause")
                    self._mem_status.set("")

            if parsed.kind == "event_outcome" and parsed.event_name:
                if self._scene_label.get() == "In round" or self._in_round:
                    self._in_round = True
                ...
                self._status.set(f"Done: {parsed.event_name} → {outcome}")
                self._refresh_options()
                if self.memory.stats_address:
