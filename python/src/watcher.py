...
class GameLogWatcher:
    def __init__(self, config: dict, project_root: Path) -> None:
        ...
        secret_path = game_data_dir(config.get("game", "Monster Prom")) / "secret_endings.json"
    ...
    def _handle_parsed(self, parsed: ParsedLine) -> None:
        ...
        if parsed.kind == "event_outcome" and parsed.event_name:
            ...
            if name.lower() in self.secret_events and self.notifications.get(
                ...
                notify(
                    "Secret Ending Event!",
                    f"{name} ({route}) — {parsed.outcome or 'triggered'}",
                    duration="long",
                )
            ...
            if self.notifications.get("game_end"):
                notify("Monster Prom", "Run finished — credits?")
            self._reset_round_state()
    ...
    def tail_file(self, log_path: Path) -> None:
        ...
        if self.skip_history:
            print("Skipping old log lines (new lines only).")
        if self.only_in_round:
            print("Notifications only during InGame_School (active round).")
        ...
        while True:
            try:
                ...
                if size < self._position:
                    # Log was cleared (game restart) — do not fire old lines again
                    self._position = size
                    self._partial = ""
                    self._reset_round_state()
                    self._round_active = False
                    print("[Log reset — waiting for a new round]")
