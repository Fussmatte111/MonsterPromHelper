...
public static class NguiOverlayView
{
    ...
    public static bool TryShow(string text)
    {
        try
        {
            ...
            if (em == null)
            {
                Plugin.Log.LogWarning("NGUI: EventManager missing — IMGUI only.");
                return false;
            }

            if (em.EventTextLabel == null)
            {
                Plugin.Log.LogWarning("NGUI: EventTextLabel missing — IMGUI only.");
                return false;
            }
            ...
            if (parent == null)
            {
                Plugin.Log.LogWarning("NGUI: UICamera missing — IMGUI only.");
                return false;
            }
            ...
            if (_built)
                Plugin.Log.LogInfo("NGUI overlay active.");
    ...
    private static bool BuildWidgets(EventManager em, Transform parent)
    {
        try
        {
            ...
            _rootGo.SetActive(false);
            _built = true;
            Plugin.Log.LogInfo("NGUI widgets created.");
            return true;
        }
        catch (System.Exception ex)
        {
