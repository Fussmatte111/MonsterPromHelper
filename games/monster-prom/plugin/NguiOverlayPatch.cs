...
public static class NguiOverlayPatch
{
    ...
    public static void TryApply()
    {
        ...
        {
            Plugin.Log.LogWarning("UICamera type not found.");
        ...
        Plugin.Log.LogWarning("Harmony patch failed — using OverlayHost for IMGUI.");
        OverlayHost.EnsureExists();
    }
    ...
    private static bool TryHarmonyPatch(Type uiCamera)
    {
        ...
        try
        {
            ...
            _patched = true;
            Plugin.Log.LogInfo("Overlay draws after NGUI (Harmony/UICamera.OnGUI).");
            return true;
        }
        catch (Exception ex)
        {
    ...
    public static void AttachHooksToUiCameras(Type uiCamera)
    {
        ...
        {
            Plugin.Log.LogWarning("No UICamera in the scene — retrying next frame.");
            return;
        }
        ...
        {
            _hookLogged = true;
            Plugin.Log.LogInfo($"OverlayDrawHook on {count} UICamera(s).");
        }
    }
