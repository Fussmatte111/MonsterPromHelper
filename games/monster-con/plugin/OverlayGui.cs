...
public static class OverlayGui
{
    ...
    public static void DrawHud(float untilTime, bool overlayOpen)
    {
        ...
        var label = Time.unscaledTime < untilTime ? "F8 = Overlay  |  F9 = Alt" : "F8 Overlay";
        var w = label.Length > 16 ? 290f : 120f;
        var ev = Event.current;
        if (ev != null && ev.type == EventType.Repaint)
        {
            if (_dimTex == null)
                _dimTex = MakeTex(new Color(0.08f, 0.12f, 0.22f, 0.92f));
            GUI.DrawTexture(new Rect(6, 6, w, 26), _dimTex);
            GUI.Label(new Rect(12, 10, w - 12, 20), label, _pickStyle!);
        }
    }
    ...
    public static void DrawFull(OverlayViewModel vm, LiveGameState live, GameBridge bridge)
    {
        ...
        GUI.Label(new Rect(0, y, innerW, 22f), "Monster Prom 4 Helper", _titleStyle!);
        y += 24f;
        ...
        if (vm.RecommendedOption > 0)
        {
            ...
            var pickTitle = "RECOMMENDED: option " + vm.RecommendedOption;
        ...
        {
            GUI.Label(new Rect(0, y, innerW, 18f), "No clear stat advantage", _mutedStyle!);
            y += 22f;
        }
        ...
        if (vm.EnginePick > 0)
        {
            GUI.Label(new Rect(0, y, innerW, 18f),
                "Game picks: option " + vm.EnginePick + " (" + vm.EngineStat + ")", _mutedStyle!);
            y += 20f;
        }
        ...
        {
            y += 6f;
            GUI.Label(new Rect(0, y, innerW, 16f), "Possible events:", _mutedStyle!);
            y += 18f;
        ...
        System.Collections.Generic.List<OptionDisplayLine> dbOptions)
    {
        ...
        {
            string statLine;
            if (dbLine.Verdict == "exchange" || (dbLine.Stat != null && dbLine.Stat.IndexOf('/') >= 0))
            {
                statLine = dbLine.Stat;
            ...
            else
            {
                ...
                statLine = mark + " " + dbLine.Stat + "=" + dbLine.Value;
                if (MonoUtil.HasText(dbLine.Hint))
                    statLine += "  |  " + Truncate(dbLine.Hint, 28);
            }
