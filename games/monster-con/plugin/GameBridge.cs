...
public sealed class GameBridge
{
    ...
    public bool TryRead(out LiveGameState state, out string error)
    {
        ...
        try
        {
            ...
            {
                if (state.InSchool)
                    error = "MainGameSceneController not ready yet.";
                else if (state.InPregame)
                    error = "PrologueSceneController not ready yet.";
            ...
            error = player != null ? "" : "No active player.";
    ...
    public bool TrySetStat(string playerName, string statKey, int value, out string error)
    {
        ...
        try
        {
            ...
            {
                error = "StatsManager or player missing";
                return false;
            }
    ...
    {
        error = "MP4 uses LI selection instead of interest points (not editable).";
        return false;
    }
