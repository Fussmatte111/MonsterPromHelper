...
public sealed class EventDb
{
    ...
    public List<string> BuildPregameSummary(LiveGameState live)
    {
        ...
        {
            lines.Add("Pregame DB missing (pregame_db.json).");
            return lines;
        }

        lines.Add("Prologue / setup (DB):");
        if (_pregamePicks.Count > 0)
            lines.Add("  Character pick = stat (see manual / Steam guide).");

        if (live.Stats.Count == 0 || _pregameChars.Count == 0)
            return lines;

        lines.Add("  LI lean from current stats:");
